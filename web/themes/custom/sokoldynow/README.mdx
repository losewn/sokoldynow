...
sokoldynow theme uses [Webpack](https://webpack.js.org) to compile and
bundle SASS and JS.
...
Go to the root of sokoldynow theme and run the following commands:
`nvm use`
...
`drush en sokoldynow -y`
`drush config-set system.theme default sokoldynow -y`
